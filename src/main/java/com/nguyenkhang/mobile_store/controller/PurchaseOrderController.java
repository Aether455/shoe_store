...
public class PurchaseOrderController {
    ...
    public ApiResponse<SimplePurchaseOrderResponse> create(@RequestBody PurchaseOrderRequest request) {
        return ApiResponse.<SimplePurchaseOrderResponse>builder()
    ...
    @GetMapping
    public ApiResponse<Page<SimplePurchaseOrderResponse>> get(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "id") String sortBy) {
        return ApiResponse.<Page<SimplePurchaseOrderResponse>>builder()
    ...
    @GetMapping("/reports")
    public ApiResponse<List<PurchaseOrderReportResponse>> reports(
            @RequestParam LocalDate start, @RequestParam LocalDate end, @RequestParam String groupBy) {
        LocalDateTime startDate = start.atStartOfDay();
        LocalDateTime endDate = end.atTime(23, 59, 59);
        return ApiResponse.<List<PurchaseOrderReportResponse>>builder()
